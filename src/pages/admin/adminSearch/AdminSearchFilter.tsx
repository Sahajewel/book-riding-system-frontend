...
const AdminSearchFilter: React.FC<AdminSearchFilterProps> = ({
  ...
  useEffect(() => {
    ...
    const filterKeys = Object.keys(params).filter(key => 
      key !== 'search' && key !== 'sortBy' && key !== 'sortOrder'
    );
  ...
  }, [debouncedSearchQuery, sortBy, sortOrder, debouncedFilters, setSearchParams, stableOnFilterChange, location.pathname]);
      ...
      {showFilters && (
        <div className="mt-4 border-t pt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {safeFilterConfig.filterSections.map(section => ( // Use safe config
              <FilterSection
                key={section.key}
                section={section}
                value={filters[section.key] || (section.type === 'multi-select' ? [] : '')}
